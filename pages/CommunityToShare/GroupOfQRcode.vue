...
<script>
	...
	export default {
		onLoad(optaion){
			...
			if(optaion.ppid !== undefined && optaion.ppid !== '' && optaion.ppid !== null){
				uni.setStorageSync('ppid', optaion.ppid);
			}
		...
		methods:{
			 longtap(){
				const token = uni.getStorageSync('token');
				 uni.request({
					url:config.webUrl+"/api/customer/incrementQrcodeClick",
					data: {'id':this.d_id},
					method:'POST',
					header:{
						'shop':"1",
			...
			getActivityPoster(){
				const token = uni.getStorageSync('token');
				const ppid = uni.getStorageSync('ppid');
				uni.request({
					...
					success: (res) => {
						if(res.data.code == 200){
							...
							this.d_id = res.data.data.id
							this.d_img = res.data.data.path
						}
					},
					fail:(el)=>{
			...
			getBecomeSilent(){
				...
				if(code === undefined || code === null || code === ''){
					location.href = 'https://open.weixin.qq.com/connect/oauth2/authorize?appid=' + config.appid + '&redirect_uri=' + encodeURIComponent(config.webUrl1 + '#/pages/CommunityToShare/GroupOfQRcode') + '&response_type=code&scope=snsapi_base&state=1#wechat_redirect'
				}else{
					this.code = code
					this.getOpenId(code)
				}				
			...
			NonSilentLogin(){
				...
				if(this.d_status != 2){
					 location.href = 'https://open.weixin.qq.com/connect/oauth2/authorize?appid='+ config.appid +'&redirect_uri=' + encodeURIComponent(config.webUrl1 + '#/pages/CommunityToShare/GroupOfQRcode') + '&response_type=code&scope=snsapi_userinfo&state=2#wechat_redirect'
				}else{
