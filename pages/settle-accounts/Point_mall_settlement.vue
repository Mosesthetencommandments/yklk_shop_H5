							...
							<view class="renRecipients-content-header" v-for="(item,index) in arr" :key="index">
								...
								<view class="centreWord">
									...
									<view class="money_box">
										<view class="godsPrice">
											￥{{item.price}}  
											<text>
												+
												<image src="../../static/images/APP-qie7_0000_bei-hui2.png" mode=""></image> {{item.integral_price}}
											</text>
										</view>
							...
							<view class="renRecipients-content-footer">
								...
								<view class="renRecipients-content-footer-choose">
									<view class="renRecipients-content-footer-choose-img">
										<view class="YBtitle">
											<img src="../../static/images/son_0019_right1.png" alt="" >
											<p :class="isChoose === false ? 'adss' : ''">使用银贝支付</p>
										</view>
										<view class="impool">
											<image src="../../static/images/APP-qie7_0001_bei-hui1.png" mode=""></image>
											<view class="">
												余额：<text>{{balance_silver}} </text> 个
											</view>
										</view>
									</view>
	...
	export default{
		...
		methods:{
				...
				if(res.data.code == 200){
					...
					for(var i = 0; i < list.length; i++){
						arr.push({
							...
							integral_price:list[i].integral_price,
			...
			async goLockOrders(){//锁定预处理订单  微信支付
				...
				const shop_goods_id = uni.getStorageSync('shop_goods_id');
				if(res.data.code == 200 && res.data.data.message === 'ok'){
					this.orders_id = res.data.data.data.id
					uni.redirectTo({
					    url: '../blankJump/blankJump?orders_id='+this.orders_id + '&shop_goods_id=' + shop_goods_id
					});
			...
			goAddress(){//去设置收货地址那
				uni.navigateTo({
					url:"/pages/address/address?d_integral=" + this.d_integral
				})
			},
			...
			async getCustomerMoneyInfo(){//获取账户余额信息
				...
				if(res.data.code == 200){
					...
					this.balance = res.data.data.balance	//余额
					this.balance_silver = res.data.data.balance_silver   //银贝
				}
			},
